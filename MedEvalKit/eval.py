...
def main():
    ...
    for eval_dataset in tqdm(args.eval_datasets):
        ...
        # Handle JSONL file paths directly
        if args.datasets_path != "hf":
            if args.datasets_path.endswith('.jsonl'):
                # If datasets_path is a JSONL file, use it directly
                eval_dataset_path = args.datasets_path
            else:
                # Otherwise, use the original logic
                eval_dataset_path = os.path.join(args.datasets_path, eval_dataset)
        else:
