...
def eval_model(args):
    ...
    # Fix model name to ensure it contains "llava" for proper image_processor loading
    if args.model_base and "llava" in args.model_base.lower():
        # Use the base model name if it contains "llava" to ensure proper loading
        model_name = os.path.basename(args.model_base)
        print(f"Using base model name for loading: {model_name}")
    ...
    for line in tqdm(dataloader, position=0, file=sys.stdout):
        ...
        gt_answer = line["reference_answer"][0] if "reference_answer" in line else ""
        answer_type = line.get("answer_type", [""])[0] if "answer_type" in line else ""

        # Use 'text' field instead of 'question' for slake dataset
