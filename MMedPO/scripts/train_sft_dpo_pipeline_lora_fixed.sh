...
# Two-Stage Training Pipeline: SFT + DPO with Same LoRA Parameters (Fixed Version)
# This script performs supervised fine-tuning followed by direct preference optimization
# Both stages use identical LoRA configuration for consistency
# Uses torchrun for distributed training instead of DeepSpeed
...
# Training parameters - FIXED DATA PATHS
SFT_DATA_PATH="/workspace/MMedPO/data/vqa-rad_dpo_weighted.json"  # Will be created
DPO_DATA_PATH="/workspace/MMedPO/data/tie_dpo_dataset_VQA.jsonn"  # Original DPO format
IMAGE_FOLDER="/workspace/MMedPO/datasets/VQA_RAD/VQA_RAD_Image_Folder"
SFT_OUTPUT_DIR="/workspace/MMedPO/MMedPO/checkpoints/sft_model_lora_vqa-rad"
DPO_OUTPUT_DIR="/workspace/MMedPO/MMedPO/checkpoints/dpo_model_lora_vqa-rad"

print_status "Starting Two-Stage Training Pipeline (SFT + DPO) with Consistent LoRA Parameters - FIXED VERSION"
print_status "=========================================================================================="
...
# Create SFT format data from DPO format data
print_status "Creating SFT format data from DPO format..."
python3 -c "
import json
...
# Convert to SFT format (only use chosen conversations)
sft_data = []
...
print_status "================================================"
print_status "Stage 1: Starting Supervised Fine-Tuning (SFT) with LoRA"
print_status "================================================"
print_status "SFT Dataset: $SFT_DATA_PATH (converted from DPO format)"
print_status "Output Directory: $SFT_OUTPUT_DIR"
print_warning "LoRA Configuration: r=128, alpha=256, dropout=0.05 (same as DPO)"
...
# Run SFT training with LoRA using torchrun (no DeepSpeed)
...
if [[ $? -eq 0 ]]; then
    print_success "SFT training with LoRA completed successfully!"
...
print_warning "LoRA Configuration: r=128, alpha=256, dropout=0.05 (consistent with SFT)"

# Set environment variables for DPO
export CUDA_VISIBLE_DEVICES=1,2,3
CUDA=1,2,3

print_status "Using current conda environment: $CONDA_DEFAULT_ENV"

# Change to the training directory
cd /workspace/MMedPO/MMedPO/train/dpo

# Run DPO training with DeepSpeed (following train_dpo_visual-text.sh configuration)
print_status "Starting DPO training with DeepSpeed..."
deepspeed --include localhost:$CUDA --master_port $((RANDOM + 30000)) ./train_dpo_weighted.py \
    --model_name_or_path /workspace/llava-med-v1.5-mistral-7b \
    --lora_checkpoint_path "$SFT_OUTPUT_DIR" \
...
print_status "================================================"
print_success "Two-Stage Training Pipeline with Consistent LoRA Completed Successfully!"
print_status "================================================"
print_success "SFT Model (LoRA): $SFT_OUTPUT_DIR"
print_success "Final DPO Model (LoRA): $DPO_OUTPUT_DIR"
print_status "LoRA Parameters Used:"
print_status "  - lora_r: 128"
print_status "  - lora_alpha: 256" 
print_status "  - lora_dropout: 0.05"
print_status "  - lora_bias: none"
