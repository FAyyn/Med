            ...
            writer.writerow(
                {
                    ...
                    "Yes/No Accuracy": file_result["yes_no_accuracy"],
                }
            )
...
def get_args():
    ...
    parser.add_argument(
        "--prediction-file",
        type=str,
        default="/path/to/prediction/file.jsonl",
        help="Path to the prediction JSONL file."
    )
    parser.add_argument(
        "--ground-truth-file",
        type=str,
        default="/workspace/MMedPO/datasets/Slake1.0/master_question.json",
    ...
    parser.add_argument(
        ...
        help="Path to the output directory for CSV results."
    )
...
if __name__ == "__main__":
    args = get_args()
    gt = load_json_array(args.ground_truth_file)

    jsonl_files = {
        "answer-file_dataset_test_vqa":{
            "result": args.prediction_file
        },
    ...
    for file_name, file_info in jsonl_files.items():
        ...
        pred = load_json_lines(result_file)
    ...
    for result in results:
        ...
        print(f"Exact Match: {result['exact_match_score']:.2f}%")
        print(f"F1 Score: {result['f1_score']:.2f}%")
        print(f"BLEU Score: {result['bleu_score']:.2f}%")
        print(f"Yes/No Accuracy: {result['yes_no_accuracy']:.2f}%")
