...
class LlavaMetaForCausalLM(ABC):
    ...
    def encode_images(self, images):
        ...
        # Ensure image features match mm_projector data type to avoid dtype mismatch
        mm_projector = self.get_model().mm_projector
        if hasattr(mm_projector, 'parameters'):
            # Get the data type of the first parameter in mm_projector
            for param in mm_projector.parameters():
                target_dtype = param.dtype
                break
            else:
                target_dtype = image_features.dtype
            # Convert image features to match mm_projector dtype
