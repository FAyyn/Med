...
class DPOTrainer(Trainer):
    ...
    def training_step(self, model, inputs):
        """
        Override training_step to handle DeepSpeed properly for DPO training.
        Manually handle backward pass to avoid no_sync context conflicts.
        """
        model.train()
        ...
        # Handle backward pass manually for DeepSpeed
        if hasattr(self.accelerator, 'deepspeed_engine_wrapped'):
            # For DeepSpeed, use the engine's backward method directly
            loss.backward()
        else:
            # For non-DeepSpeed setups, use standard backward
            if self.use_apex:
