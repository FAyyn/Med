...
class TrainingArguments(transformers.TrainingArguments):
    ...
    beta: float = field(default=0.5, metadata={"help": "DPO beta; higher keeps closer to SFT policy."})
    loss_type: str = field(default="sigmoid", metadata={"help": "DPO loss type: 'sigmoid' or 'hinge'."})
...
def compute_weight(
    ...
    """
    Compute DPO weight from raw TIE-based composite score.

    Parameters:
    -----------
    S_raw : array-like
        Raw composite scores for each sample.
    mean, std : float or None
        Dataset-wide mean and std for normalization (if None, computed from batch).
    beta_T : float
        Temperature scaling factor for sigmoid/softsign.
    tau : float
        Center shift for sigmoid (recommended 0.2~0.3).
    kappa : float
        Power scaling factor (0.5~1.0 recommended).
    w_min, w_max : float
        Clipping bounds for final weights.
    method : str
        "sigmoid" (default) or "softsign" for alternative mapping.

    Returns:
    --------
    weights : np.ndarray
        Normalized and clipped weights in [w_min, w_max].
    """
    ...
    # Step 2: Nonlinear mapping
    if method == "softsign":
        p = 0.5 * (1 + (S_norm - tau) / (1 + np.abs(S_norm - tau)))
    else:
        p = 1.0 / (1.0 + np.exp(-beta_T * (S_norm - tau)))

    # Step 3: Power transform
    w = np.power(p, kappa)

    # Step 4: Clip to safe range
...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(
        ...
        # Compute dataset-wide statistics for weighted_score for stable normalization
        try:
            raw_scores = [float(s.get("weighted_score", 0.0)) for s in self.list_data_dict if "weighted_score" in s]
            if len(raw_scores) == 0:
                self.weight_mean = 0.0
                self.weight_std = 1.0
            else:
                self.weight_mean = float(np.mean(raw_scores))
                # add small epsilon to std to avoid divide-by-zero
                self.weight_std = float(np.std(raw_scores) + 1e-8)
            rank0_print(f"[DPO] weight stats: mean={self.weight_mean:.6f}, std={self.weight_std:.6f}, n={len(raw_scores)}")
        except Exception as e:
            # Fallback if stats computation fails
            warnings.warn(f"Failed computing weight stats, using defaults. Error: {e}")
            self.weight_mean = 0.0
            self.weight_std = 1.0
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        # Map raw weighted_score to stable training weight using compute_weight
        raw_w = float(sources[0].get("weighted_score", 0.0))
        dpo_weight = float(
            compute_weight(
                [raw_w],
...
def train():
    ...
    if model_args.vision_tower is not None:
        ...
        vision_tower.to(dtype=target_dtype, device=training_args.device)
        
        # Ensure all vision tower submodules are in the correct dtype
    ...
    # 仅当我们在本轮训练中新建 LoRA 配置时才传入 peft_config；
    # 如果通过 lora_checkpoint_path 加载了现有 LoRA，则避免在 Trainer 内再次包裹。
    peft_config_to_pass = locals().get('lora_config', None)
    ...
    # Fix both main model and reference model vision tower and mm_projector data types
    target_dtype = torch.bfloat16 if training_args.bf16 else torch.float16
    
    # Fix main model vision tower data type
    if hasattr(model, 'get_vision_tower') and model.get_vision_tower() is not None:
        main_vision_tower = model.get_vision_tower()
        main_vision_tower.to(target_dtype)
        # Ensure all submodules are also cast to the target dtype
        for module in main_vision_tower.modules():
            if hasattr(module, 'weight') and module.weight is not None:
                module.weight.data = module.weight.data.to(target_dtype)
            if hasattr(module, 'bias') and module.bias is not None:
                module.bias.data = module.bias.data.to(target_dtype)
    
    # Fix main model mm_projector data type
    if hasattr(model, 'get_model') and hasattr(model.get_model(), 'mm_projector') and model.get_model().mm_projector is not None:
        main_mm_projector = model.get_model().mm_projector
        main_mm_projector.to(target_dtype)
        # Ensure all submodules are also cast to the target dtype
        for module in main_mm_projector.modules():
            if hasattr(module, 'weight') and module.weight is not None:
                module.weight.data = module.weight.data.to(target_dtype)
            if hasattr(module, 'bias') and module.bias is not None:
                module.bias.data = module.bias.data.to(target_dtype)
    
    # Fix reference model vision tower data type to match training model
    if hasattr(trainer, 'ref_model') and trainer.ref_model is not None:
        if hasattr(trainer.ref_model, 'get_vision_tower') and trainer.ref_model.get_vision_tower() is not None:
            ref_vision_tower = trainer.ref_model.get_vision_tower()
            ref_vision_tower.to(target_dtype)
            # Ensure all submodules are also cast to the target dtype
            for module in ref_vision_tower.modules():
                if hasattr(module, 'weight') and module.weight is not None:
                    module.weight.data = module.weight.data.to(target_dtype)
                if hasattr(module, 'bias') and module.bias is not None:
                    module.bias.data = module.bias.data.to(target_dtype)
        
        # Fix reference model mm_projector data type to match training model
        if hasattr(trainer.ref_model, 'get_model') and hasattr(trainer.ref_model.get_model(), 'mm_projector') and trainer.ref_model.get_model().mm_projector is not None:
            ref_mm_projector = trainer.ref_model.get_model().mm_projector
            ref_mm_projector.to(target_dtype)
            # Ensure all submodules are also cast to the target dtype
            for module in ref_mm_projector.modules():
                if hasattr(module, 'weight') and module.weight is not None:
                    module.weight.data = module.weight.data.to(target_dtype)
                if hasattr(module, 'bias') and module.bias is not None:
                    module.bias.data = module.bias.data.to(target_dtype)
